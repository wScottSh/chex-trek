...
- **Open questions (binary behavior, need an in-game check).**
  ...
  - Objectives are re-added in the order their `Restore` events fire, so after a load an objective may get a different number (and map icon slot) than before.
    - Confirmed on the dhewm3 port (spec #31, `tools/test-objectives.sh`): with only `sf_923`'s `trigger_objective_2` active, it comes back in slot 1, not 2. `nextObjective` is not saved and the new `idPlayer`'s constructor sets it to 0 (`Restore` does not call `Init`). Kept as is (original behavior); the scenario asserts slot 1.

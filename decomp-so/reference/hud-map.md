...
- **Edits inside stock functions (out of scope for spec #16, recorded as leads).** A scan of all code for `[reg + disp]` operands in +0x1e30..+0x1ea3 found these users besides this group (indexed forms were not scanned):
  ...
  - `idPlayer::Save` (0x152d88-0x152e8f) writes `mapScale`, `mapRadius`, `unknown1e5c`, `revealDistance`, `mapWidth`, `mapHeight`, `mapCoords`, `mapMaterial`, then all of `hudmap_alpha`, one `WriteByte` per byte. `idPlayer::Restore` (0x168b19-0x168bdd) reads them back in the same order. `mapControl`, `mapView`, `lastRevealOrigin` and `mapLevels` are not saved. Only `initHudMap` sets `mapLevels`, and only `idPlayer::Spawn` calls it; the constructors do not set them.
    - **Ported by spec #39, two port choices.** `hudmap_alpha` is written and read with one bulk `savefile->Write` / `Read` (same bytes, same order as the binary's per-byte loop; the SDK's usual form for byte arrays). And, unlike the binary, the `idPlayer` constructor also sets `mapControl` / `mapView` / `lastRevealOrigin` to 0, `unknown1e5c` to -1 and `mapLevels` to `initHudMap`'s no-key defaults (level 0 at `MIN_WORLD_COORD`, 1-4 at `MAX_WORLD_COORD`): a savegame load (`idGameLocal::InitFromSaveGame`) builds a new `idPlayer` and runs neither `Spawn` nor `Init`, so these unsaved members held leftover heap memory. Seen live: a garbage `mapLevels[ 0 ]` made `HudMapLevel` warn "Location below lowest MapLevel" after every save/load. A no-op on a normal spawn, where `initHudMap` and `Init` overwrite them.
...
- **Open questions (need an in-game check or more binary work).**
  ...
  - Whether `mapLevels` are valid after loading a save: they are not saved, and whether `idPlayer::Spawn` (hence `initHudMap`) runs on a load was not checked.
    - Checked on the dhewm3 port (spec #39): `Spawn` does not run on a load, so they are not valid there; see the `idPlayer::Save` lead above for the port's constructor defaults. Whether the original binary shows the same symptom was not checked.

...
- **Ported by spec #43 (`game/Trail.h` / `Trail.cpp`, the leads above): port choices and fixes, not in the binary.**
  - `renderEntity.callbackData` (a `void *` the SDK reserves for the callback) holds the `mkTrail *`, instead of the `reinterpret_cast<int>( this )` into `entityNum` (spec #43 AC2: no pointer-to-int casts). `ModelCallback` casts it back from `void *`.
  - `addNewAnchor`'s no-owner branch returns false when there is no anchor yet, instead of reading `anchors[ -1 ]` (the open question below; fixed as a crash risk, spec #28's "unless they crash" carve-out).
  - `idGameLocal::MapClear` deletes every trail left in `trails`. With `fadeTime` -1 no trail deletes itself, so otherwise each map's trails leak into the next, and a trail still fading across a map change would update a render entity of the old render world. Reasoned from the code, not reproduced.
  - The `idActor` constructor sets `hasTrail` false and `trail` NULL, and `~idActor` tests `hasTrail && trail`. `idActor::Save` / `Restore` are not edited, so an actor loaded from a save has no trail (whether the binary saves them is still unchecked, below).
...
- **Open questions (binary behavior, need an in-game check).**
  - `lastPos` is never initialized (not by the constructor, not by `Spawn`). The first `Think` compares the owner's origin with whatever the allocation held; `Spawn` also sets `TH_UPDATEVISUALS`, so the first `Present` traces from that value if the distance test fails (for example on a NaN).
    - Checked in game on the dhewm3 port (spec #28, whole-PR review): on `sf_923` about half of the map's flemoid trails start with a garbage `lastPos` (NaN, or components near 1e38 whose squared length overflows). `LengthFast() > updateDist` is then false on every frame, so those trails never update and never lay an anchor for the rest of the level; the others anchor normally. Kept as is (an original-mod bug that doesn't crash, spec #28 Out of Scope). `tools/test-trails.sh` asserts anchors only on the trails that did initialize.

...
- **spawnArgs, cross-checked with `def/` and `maps/`.**
  ...
  - Actors: `def/chex_monster_flemoid.def` sets `"hasTrail" "1"`, `"trailDef" "trail"`. `def/monster_default.def` and one entity in `maps/sf_923.map` set `"hasTrail" "0"`.
...
- **Ghidra artifacts.**
  ...
  - The D0 destructor passes `pmVar3` (a leftover register) to `operator_delete`; it is `idClass::operator delete( this )`.

...
- **Open questions.** Whether `takeEnvShots` and `atSpawn` shots work in game (the render call outside the frame, above). Why `atSpawn` waits 250 ms (probably so the map has rendered once). Both need an in-game check.
  - Partly checked on the dhewm3 port (spec #44, `tools/test-env-shots.sh`): `takeEnvShots` writes all six faces at the entity's `size`. That does not show whose view they are: a `tr.primaryView` left over from the player's last frame would give the same files. `atSpawn` was not exercised. The leak is kept (original-mod bug, doesn't crash).

...
void idGameLocal::Init( void ) {
	...
	// chextrek: prove chextrek.dll (not base.dll) loaded before the default scripts are parsed -
	// if the mod's scripts fail to compile (e.g. the known "Unknown event" failure #29's harness
	// checks for) game init aborts below and never reaches a point where a console command could
	// run, so this can't wait for someone to type/exec "chextrek_dump". See ChexTrekDump.h.
